...
button.addEventListener("click", function () {
  ...
  fetch(
    `https://www.thecolorapi.com/scheme?hex=${selectedColor.slice(1)}&mode=${mode.value}&count=${count}`
  )
    ...
    .then((data) => {
      ...
      document.querySelectorAll('.colors > div').forEach((block, i) => {
        block.addEventListener('click', function () {
          const hex = document.querySelectorAll('.bottom-container > div')[i].textContent.trim();
